...
# residual blocks
for i in range(16):
    crt_net['sft_branch.{:d}.conv0.weight'.format(i)] = pretrained_net[
        'model.1.sub.{:d}.res.0.weight'.format(i)]
    crt_net['sft_branch.{:d}.conv0.bias'.format(i)] = pretrained_net[
        'model.1.sub.{:d}.res.0.bias'.format(i)]
    crt_net['sft_branch.{:d}.conv1.weight'.format(i)] = pretrained_net[
        'model.1.sub.{:d}.res.2.weight'.format(i)]
    crt_net['sft_branch.{:d}.conv1.bias'.format(i)] = pretrained_net[
        'model.1.sub.{:d}.res.2.bias'.format(i)]
